...
largest i cbt = 
    let
        indices = [i,2*i+1,2*i+2]
        values = List.map (\x -> (getForList cbt x)) indices
    in
        case (List.indexedMap (\t a -> (t,a)) values  |> LE.maximumBy Tuple.second) of 
            Just (idx,a) -> (getForList indices idx)
            Nothing -> -1
...
largestAll i cbt = 
    ...
    in
        case (List.maximum values) of 
            Just v -> List.indexedMap (\t a -> (t,a)) values 
